...
class ProductDetailView(View):
    ...
    def get(self , request ,*args , **kwargs):
        comments = self.product_instance.pcomments.filter(is_reply = False,status=Comment.Status.APPROVED)
        return render(request , "home/detail.html" , {"product" :self.product_instance , 'comments': comments
    ...
    def post(self , request , *args , **kwargs):
        ...
        if form.is_valid():
            ...
            new_comment.Status = Comment.Status.PENDING
            new_comment.user = request.user
            new_comment.product =self.product_instance
            new_comment.save()
            messages.success(request , "کامنت شما با موفقیت ارسال شد و در حال بررسی می باشد." , "success")
...
class ProductAddReplyView(View):
    ...
    def post(self , request , product_id, comment_id):
        ...
        if form.is_valid():
            ...
            reply.Status = reply.Status.PENDING
            reply.user= request.user
            reply.product = product
            reply.reply = comment
            reply.is_reply = True
            reply.save()
            messages.success(request  , "کامنت شما با موفقیت ارسال شد و در حال بررسی است" , "success")
